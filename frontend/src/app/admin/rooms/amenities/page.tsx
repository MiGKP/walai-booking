...
export default function AmenitiesPage() {
  ...
  const fetchAmenities = async () => {
    ...
    try {
      ...
      toast.error('ไม่สามารถโหลดข้อมูลสิ่งอำนวยความสะดวกได้ (หรือยังไม่มี API)');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateAmenity = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/rooms/amenity/${editingAmenity.id}`, { name: editingAmenity.name });
    ...
    <div className="min-h-screen pt-16 bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">จัดการสิ่งอำนวยความสะดวก</h1>
            <p className="text-gray-500 mt-1">เพิ่มและแก้ไขสิ่งอำนวยความสะดวกในห้องพัก</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 mb-6 bg-white p-2 rounded-xl shadow-sm w-fit border border-gray-100">
          ...
          <Link href="/admin/rooms/amenities" className="px-4 py-2 rounded-lg text-sm font-medium bg-indigo-50 text-indigo-700">สิ่งอำนวยความสะดวก</Link>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="card p-6 lg:col-span-1 h-fit">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit" className="btn-primary w-full mt-2 bg-indigo-600 hover:bg-indigo-700">บันทึกข้อมูล</button>
            </form>
          </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-white border-b border-gray-100">
                    <tr>
                      <th className="text-left px-4 py-3 font-semibold text-gray-600">ไอดี</th>
                    ...
                    {loading ? (
                      ...
                      amenities.map((am: any) => (
                        <tr key={am.id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-4 py-3 font-medium text-gray-900">{am.id}</td>
                          <td className="px-4 py-3 text-gray-900">{am.name}</td>
                          <td className="px-4 py-3">
                            <span className={`px-2 py-1 rounded-full text-xs font-bold ${am.status ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{am.status ? 'ใช้งานอยู่' : 'ปิดใช้งาน'}</span>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-end gap-2">
                              <button onClick={() => openEditModal(am)} className="p-1.5 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors" title="แก้ไข">
                                <Edit2 size={16} />
                              </button>
                              <button onClick={() => handleDelete(am.id, am.name)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="ลบ">
                                <Trash2 size={16} />
                              </button>
                            </div>
                          </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
              <h3 className="text-lg font-bold text-gray-900">แก้ไขสิ่งอำนวยความสะดวก</h3>
              <button onClick={() => { setShowEditModal(false); setEditingAmenity(null); }} className="p-1 hover:bg-gray-200 rounded-full transition-colors">
                <X size={20} className="text-gray-500" />
              </button>
            </div>
            <form onSubmit={handleUpdateAmenity} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">ชื่อสิ่งอำนวยความสะดวก</label>
                <input type="text" required className="input-field" value={editingAmenity.name} onChange={(e) => setEditingAmenity({ ...editingAmenity, name: e.target.value })} />
              </div>
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => { setShowEditModal(false); setEditingAmenity(null); }} className="flex-1 px-4 py-2 border border-gray-300 rounded-xl text-gray-700 font-medium hover:bg-gray-50 transition-colors">ยกเลิก</button>
                <button type="submit" className="flex-1 btn-primary bg-indigo-600 hover:bg-indigo-700">บันทึก</button>
              </div>
            </form>
          </div>
        </div>
